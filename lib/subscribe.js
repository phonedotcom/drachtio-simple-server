...
const {parseAor, parseEventHeader, getDefaultSubscriptionExpiry, forwardStoredMsgs} = require('./utils');
...
function initial(req, res) {
  ...
  db.addSubscription(req.event, req.expiry)
    ...
    .then(() => {
      ...
      logger.info(`subscribe#initial: retrieving stored messages for ${req.event.subscriber}`);
      return forwardStoredMsgs(req.srf, req.event.subscriber, db);
    })
    .catch((err) => {
...
function refresh(req, res, dlg, subscription) {
  ...
  return db.removeSubscription(subscription)
    ...
    .then(() => {
      return res.send(202, {headers: {'Expires': expiry}});
    })
    .then(() => {
...
function validate(req, res) {
  ...
  if (config.has('methods.subscribe.expires.max') && req.expiry > config.get('methods.subscribe.expires.max')) {
    ...
    logger.info(`subscribe#validate: reducing Expires value to ${req.expiry}`);
  }
