 ...
 *
 * III. Messages
 * When we receive a message for an aor that is offline we store it in redis.
 * We keep a list of the unsent messages for an aor
 *
 * The key to the list is messages:${aor}
 *
 * Each element in the list will be a unique id, called ${uuid} here that refers to a has
 * ${uuid} => {from, time, content type, content}
 */
...
class Db extends Emitter {
  ...
  */
  ...
  saveMessage(from, to, type, content, fromHdr, toHdr, toUser) {
    const uuid = translator.new();
    const key = makeMessagesKey(to);

    return new Promise((resolve, reject) => {
      this.client.multi()
        .hmset(uuid, {
          from, to, type, content, fromHdr, toHdr, toUser,
          time: new Date()
        })
        .rpush(key, uuid)
        .exec((err, replies) => {
          if (err) return reject(err);
          debug(`Db#saveMessage: replies ${JSON.stringify(replies)}`);
          resolve(replies);
        });
    });
  }
  ...
  retrieveMessages(to) {
    ...
    return new Promise((resolve, reject) => {
      debug(`Db#retrieveMessages: retrieving messages for ${to} using key ${key}`);
      this.client.multi()
        .lrange(key, 0, -1)
        .del(key)
        .exec((err, replies) => {
          if (err) return reject(err);
          debug(`Db#retrieveMessages: replies ${JSON.stringify(replies)}`);
          ...
          async.whilst(
            () => idx < keys.length,
            (callback) => {
              this.client.hgetall(keys[idx++], (err, obj) => {
                msgs.push(obj);
                callback();
              });
            },
            (err) => {
              ...
              debug(`retrieved messages for sending to ${to}: ${JSON.stringify(msgs)}`);
              resolve(msgs);
            }
          );
        });
